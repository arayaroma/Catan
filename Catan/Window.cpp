...
void Window::printTitleScreenLabels() {
  ...
  image3.draw_text(returnWindow->get_x_position(), returnWindow->get_y_position(),
      returnWindow->mod_get_text(), ORANGE, transparent, opacity,
      returnWindow->get_font_size());
...
void Window::showDisplay() {
  ...
  while (!title_display.is_closed()) {
    ...
    if (title_display.button()) {
        if (clickAboutLabel(title_display.mouse_x(), title_display.mouse_y(), playLabel)) {
            std::cout << "1" << std::endl;
            showGameDisplay();
        }
        else if (clickAboutLabel(title_display.mouse_x(), title_display.mouse_y(), about)) {
            showInfoGame();
        }
...
void Window::showInfoGame() {
    ...
    CImgDisplay info_display(catan_title_display_width, catan_title_display_height,
        catan_window_title, 3, false, false);
    ...
    while (!info_display.is_closed()) {
        ...
        if (info_display.button()) {
            std::cout << "1" << std::endl;
            if (clickAboutLabel(info_display.mouse_x(), info_display.mouse_y(), returnWindow)) {
                info_display.close();
                showDisplay();
            }
        }
